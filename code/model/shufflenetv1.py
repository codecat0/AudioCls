...
class ShuffleNetUnits(nn.Module):
    def __init__(self, in_channel, out_channel, stride, groups):
        ...
        self.bottleneck = nn.Sequential(
            ...
            # 3x3 DWConv
            ConvBNRelU(in_channel=mid_channel, out_channel=mid_channel, kernel_size=3, stride=stride,
                       groups=mid_channel),
            # 1x1 GConv
...
class ShuffleNet(nn.Module):
    def __init__(self, planes, layers, groups, num_classes=1000):
        ...
        self.stage2 = self._make_layer(in_channel=24, out_channel=planes[0], groups=groups, block_num=layers[0],
                                       is_stage2=True)
        self.stage3 = self._make_layer(in_channel=planes[0], out_channel=planes[1], groups=groups, block_num=layers[1],
                                       is_stage2=False)
        self.stage4 = self._make_layer(in_channel=planes[1], out_channel=planes[2], groups=groups, block_num=layers[2],
                                       is_stage2=False)
